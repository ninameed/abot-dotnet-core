...
namespace Abot.Tests.Unit.Core
{
    ...
    public class CrawlDecisionMakerTest
    {
        ...
        public async Task ShouldDownloadPageContent_DownloadableContenttypesWithSpaces_TrimsSpaces_ReturnsTrue()
        {
            ...
            CrawlDecision result = _unitUnderTest.ShouldDownloadPageContent(await new PageRequester(crawlConfiguration).MakeRequestAsync(valid200StatusUri), _crawlContext);
        ...
        public void ShouldRecrawlPage_NullWebException_ReturnsFalse()
        {
            ...
            Assert.IsFalse(result.Allow);
            Assert.AreEqual("HttpRequestException did not occur", result.Reason);
            Assert.IsFalse(result.ShouldHardStopCrawl);
            Assert.IsFalse(result.ShouldStopCrawl);
